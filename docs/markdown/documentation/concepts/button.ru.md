...
3. Вывод возвращается в виде блока кода с кодом завершения и временем
    выполнения. Короткий вывод остаётся в чате одним или несколькими
    сообщениями; более длинный вывод может прийти файлом `.txt` (см.
    [Длинные результаты файлом](#long-results-as-a-file)).
...
На кнопке `output` не обязателен. Если его нет, кнопка берёт корневой режим
(`auto`, если вы его не меняли): короткий результат остаётся сообщениями в чате,
а результат длиннее двух сообщений приходит одним файлом `.txt`. Указывайте
`output` на кнопке только если эта кнопка всегда должна слать файл или всегда
оставаться текстом. `max_output_messages` на кнопке задать нельзя.

Задайте `output: file` на кнопке, которая всегда печатает много (например логи),
или `output: text`, чтобы оставить старое поведение только для этой кнопки.

Корневые ключи `output` и `max_output_messages` описаны в разделе
[Конфигурация → Сколько выводимых команд вы видите](../configuration.md#how-much-command-output-you-see).
Страница [Управление выводом](../output-control.md) проходит все сочетания
корня и кнопки, по одному примеру на каждое.
...
- :material-export-variant:{ .middle } __Управление выводом__

    ---

    Сообщения или файл `.txt` и как кнопка переопределяет корень.

    [:octicons-arrow-right-24: Управление выводом](../output-control.md)
